...
Form::Form(QWidget *parent)
    : QWidget(parent)
    , ui(new Ui::Form)
    , m_DobotMove(&m_RobotConnector)
    , m_Dashboard(&m_RobotConnector)
{
    ui->setupUi(this);
...
void Form::Connect()
{
    ...
    PrintLog("Connecting...");
    std::thread thd([=]{
        if (!m_RobotConnector.Connect(strIp.toStdString(), iPortDashboard))
        {
...
void Form::on_btnEnable_clicked()
{
    ...
    PrintLog(QString::asprintf("send to %s:%hu: %s()", m_RobotConnector.GetIp().c_str(),
                               m_RobotConnector.GetPort(), bEnable? "EnableRobot" : "DisableRobot"));
    std::thread thd([=]{
        ...
        PrintLog(QString::asprintf("Receive From %s:%hu: %s", m_RobotConnector.GetIp().c_str(),
                                   m_RobotConnector.GetPort(), ret.c_str()));
    });
    thd.detach();
}
...
{
    PrintLog(QString::asprintf("send to %s:%hu: ClearError()", m_RobotConnector.GetIp().c_str(), m_RobotConnector.GetPort()));
    std::thread thd([this]{
...
void Form::on_btnConfirmSpeed_clicked()
{
    ...
    PrintLog(QString::asprintf("send to %s:%hu: SpeedFactor(%d)", m_RobotConnector.GetIp().c_str(), m_RobotConnector.GetPort(),iValue));
    std::thread thd([=]{
...
void Form::on_btnConfirmDO_clicked()
{
    ...
    PrintLog(QString::asprintf("send to %s:%hu: DigitalOutputs(%d,%s,2000)", m_RobotConnector.GetIp().c_str(),
                               m_RobotConnector.GetPort(),idx,bIsOn?"on":"off"));
    std::thread thd([=]{
        std::string ret = m_Dashboard.DigitalOutputs(idx, bIsOn,2000);
        PrintLog(QString::asprintf("Receive From %s:%hu: %s", m_RobotConnector.GetIp().c_str(),
                                   m_RobotConnector.GetPort(), ret.c_str()));
    });
    thd.detach();
...
void Form::on_btnMovJ_clicked()
{
    ...
    PrintLog(QString::asprintf("send to %s:%hu: MovJ(%s)", m_RobotConnector.GetIp().c_str(),
                               m_RobotConnector.GetPort(),pt.ToString().c_str()));
    std::thread thd([=]{
...
void Form::on_btnMovL_clicked()
{
    ...
    PrintLog(QString::asprintf("send to %s:%hu: MovL(%s)", m_RobotConnector.GetIp().c_str(),
                               m_RobotConnector.GetPort(),pt.ToString().c_str()));
    std::thread thd([=]{
...
{
    PrintLog(QString::asprintf("send to %s:%hu: MoveJog(%s)", m_RobotConnector.GetIp().c_str(),
                               m_RobotConnector.GetPort(),str.toStdString().c_str()));
    std::thread thd([=]{
...
{
    PrintLog(QString::asprintf("send to %s:%hu: MoveJog()", m_RobotConnector.GetIp().c_str(),
                               m_RobotConnector.GetPort()));
    std::thread thd([=]{
...
void Form::ShowDataResult()
{
    ...
    {//第一次填充数据，免得用的时候一个一个输入
        ui->lineEditX->setText(QString::asprintf("%.3f", m_Feedback.GetFeedbackData().ToolVectorActual[0]));
        ui->lineEditY->setText(QString::asprintf("%.3f", m_Feedback.GetFeedbackData().ToolVectorActual[1]));
        ui->lineEditZ->setText(QString::asprintf("%.3f", m_Feedback.GetFeedbackData().ToolVectorActual[2]));
        ui->lineEditRX->setText(QString::asprintf("%.3f", m_Feedback.GetFeedbackData().ToolVectorActual[3]));
        ui->lineEditRY->setText(QString::asprintf("%.3f", m_Feedback.GetFeedbackData().ToolVectorActual[4]));
        ui->lineEditRZ->setText(QString::asprintf("%.3f", m_Feedback.GetFeedbackData().ToolVectorActual[5]));
    }
