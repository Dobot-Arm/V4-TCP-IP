...
namespace CSharpTcpDemo.com.dobot.api
{
    class Feedback : DobotClient
    {
        ...
        private void ParseData(byte[] buffer)
        {
            ...
            feedbackData.ProgramState = BitConverter.ToDouble(buffer, iStartIndex);
            iStartIndex += 8;

            feedbackData.SafetyStatus = BitConverter.ToDouble(buffer, iStartIndex);
            iStartIndex += 8;
            ...
            for (int i = 0; i < feedbackData.ActualTCPForce.Length; ++i)
            {
                feedbackData.ActualTCPForce[i] = BitConverter.ToDouble(buffer, iStartIndex);
                iStartIndex += 8;
            }
            ...
            feedbackData.CollisionState = buffer[iStartIndex];
            iStartIndex += 1;
            feedbackData.ArmApproachState = buffer[iStartIndex];
            iStartIndex += 1;
            feedbackData.J4ApproachState = buffer[iStartIndex];
            iStartIndex += 1;
            feedbackData.J5ApproachState = buffer[iStartIndex];
            iStartIndex += 1;
            feedbackData.J6ApproachState = buffer[iStartIndex];
            iStartIndex += 1;
        ...
        public string ConvertRobotMode()
        {
            switch (feedbackData.RobotMode)
            {
                ...
                case FeedbackData.ROBOT_MODE_POWEROFF:
                    return "ROBOT_MODE_POWEROFF";
                ...
                case FeedbackData.ROBOT_MODE_SINGLE_MOVE:
                    return "ROBOT_MODE_SINGLE_MOVE";
                case FeedbackData.ROBOT_MODE_ERROR:
                    return "ROBOT_MODE_ERROR";
                case FeedbackData.ROBOT_MODE_PAUSE:
                    return "ROBOT_MODE_PAUSE";
                case FeedbackData.ROBOT_MODE_COLLISION:
                    return "ROBOT_MODE_COLLISION";
            }
