...
namespace CSharpTcpDemo
{
    ...
    {
        private RobotConnector mRobotConnector = new RobotConnector();
        private Feedback mFeedback = new Feedback();
        private DobotMove mDobotMove = null;
        ...
        public MainForm()
        {
            ...
            mDobotMove = new DobotMove(mRobotConnector);
            mDashboard = new Dashboard(mRobotConnector);
            ...
            mRobotConnector.NetworkErrorEvent += new DobotClient.OnNetworkError(this.OnNetworkErrorEvent_RobotConnector);
        ...
        private void MainForm_FormClosed(object sender, FormClosedEventArgs e)
        {
            ...
            if (this.mRobotConnector != null)
            {
                this.mRobotConnector.Disconnect();
            }
        }
        ...
        {
            PrintLog(string.Format("send to {0}:{1}: MoveJog({2})", mRobotConnector.IP, mRobotConnector.Port,str));
            Thread thd = new Thread(() => {
                string ret = mDobotMove.MoveJog(str);
                PrintLog(string.Format("Receive From {0}:{1}: {2}", mRobotConnector.IP, mRobotConnector.Port, ret));
            });
            thd.Start();
        }
        ...
        {
            PrintLog(string.Format("send to {0}:{1}: MoveJog()", mRobotConnector.IP, mRobotConnector.Port));
            Thread thd = new Thread(() => {
        ...
        private void OnNetworkErrorEvent_RobotConnector(DobotClient sender, SocketError iErrCode)
        {
            if (mIsManualDisconnect) return;
            this.BeginInvoke(new Action(() => {
                string strIp = textBoxIP.Text;
                int iPort = Parse2Int(this.textBoxDashboardPort.Text);
                DoNetworkErrorEvent(mRobotConnector, strIp, iPort);
            }));
        }
        ...
        private void Connect()
        {
            ...
            PrintLog("Connecting...");
            Thread thd = new Thread(() => {
                if (!mRobotConnector.Connect(strIp, iPortDashboard))
                {
        ...
        private void btnEnable_Click(object sender, EventArgs e)
        {
            ...
            PrintLog(string.Format("send to {0}:{1}: {2}()", mRobotConnector.IP, mRobotConnector.Port, bEnable? "EnableRobot" : "DisableRobot"));
            Thread thd = new Thread(() => {
        ...
        {
            PrintLog(string.Format("send to {0}:{1}: ClearError()", mRobotConnector.IP, mRobotConnector.Port));
            Thread thd = new Thread(() => {
        ...
        private void btnSpeedConfirm_Click(object sender, EventArgs e)
        {
            ...
            PrintLog(string.Format("send to {0}:{1}: SpeedFactor({1})", mRobotConnector.IP, mRobotConnector.Port, iValue));
            Thread thd = new Thread(() => {
        ...
        private void btnMovJ_Click(object sender, EventArgs e)
        {
            ...
            PrintLog(string.Format("send to {0}:{1}: MovJ({2})", mRobotConnector.IP, mRobotConnector.Port,pt.ToString()));
            Thread thd = new Thread(() => {
        ...
        private void btnMovL_Click(object sender, EventArgs e)
        {
            ...
            PrintLog(string.Format("send to {0}:{1}: MovL({2})", mRobotConnector.IP, mRobotConnector.Port, pt.ToString()));
            Thread thd = new Thread(() => {
                if (mFeedback.IsEnabled())
                {
                    mDashboard.EnableRobot();
        ...
        private void btnDOInput_Click(object sender, EventArgs e)
        {
            ...
            PrintLog(string.Format("send to {0}:{1}: DigitalOutputs({2},{3},2000)", mRobotConnector.IP, mRobotConnector.Port,
                idx, bIsOn));
            Thread thd = new Thread(() => {
                string ret = mDashboard.DigitalOutputs(idx, bIsOn, 2000);
                PrintLog(string.Format("Receive From {0}:{1}: {2}", mRobotConnector.IP, mRobotConnector.Port, ret));
            });
            thd.Start();
        }
        ...
        private void btnDOInputIO_Click(object sender, EventArgs e)
        {
            ...
            PrintLog(string.Format("send to {0}:{1}: ToolDO({2},{3})", mRobotConnector.IP, mRobotConnector.Port,
                idx, bIsOn));
            Thread thd = new Thread(() => {
        ...
        private void ShowDataResult()
        {
            ...
            if (null != mFeedback.feedbackData.ToolVectorActual && mFeedback.feedbackData.ToolVectorActual.Length >= 6)
            {
                ...
                {//第一次填充数据，免得用的时候一个一个输入
                    ...
                    this.textBoxRy.Text = string.Format("{0:F3}", mFeedback.feedbackData.ToolVectorActual[4]);
                    this.textBoxRz.Text = string.Format("{0:F3}", mFeedback.feedbackData.ToolVectorActual[5]);
                }
            }
